...
Universal Compare 是一个基于多智能体 AI 架构的智能比较工具，能够对任意两个实体（产品、概念、技术等）进行深度、多维度的对比分析。
...
Universal Compare 的核心是一个精心设计的 4 阶段多智能体 AI Pipeline，每个阶段由专门的 Agent 负责特定任务。
...
└────────────────────────────┬────────────────────────────────────────┘
                             ...
                             ▼
┌─────────────────────────────────────────────────────────────────────┐
│                    PHASE 1: 双轨研究 (Dual-Track Research)          │
│                                                                     │
│  ┌─────────────────────┐              ┌─────────────────────┐      │
│  │ ResearcherAgent A   │              │ ResearcherAgent B   │      │
│  │                     │              │                     │      │
│  │ • Web Search        │   并发执行    │ • Web Search        │      │
...
└─────────────────────────────────────────────────────────────────────┘
                             ...
                             ▼
┌─────────────────────────────────────────────────────────────────────┐
│              PHASE 2: 框架架构 (Framework Architecture)             │
│                                                                     │
│                   ┌─────────────────────┐                          │
│                   │  ArchitectAgent     │                          │
│                   │                     │                          │
...
└─────────────────────────────────────────────────────────────────────┘
                             ...
                             ▼
┌─────────────────────────────────────────────────────────────────────┐
│           PHASE 3: 多维度分析 (Multi-Dimensional Analysis)          │
│                                                                     │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐             │
│  │AnalystAgent 1│  │AnalystAgent 2│  │AnalystAgent 3│             │
│  │              │  │              │  │              │             │
...
│                           │                                        │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐             │
│  │AnalystAgent 4│  │AnalystAgent 5│  │AnalystAgent 6│             │
│  │              │  │              │  │              │             │
│  │ Dimension 4  │  │ Dimension 5  │  │ Dimension 6  │             │
│  └──────────────┘  └──────────────┘  └──────────────┘             │
│                                                                     │
│  ⚠️  并发限制: 每次最多 3 个并发请求（避免 API 限流）                 │
│                                                                     │
│  每个 AnalystAgent 输出:                                            │
│  • item_a_summary (实体 A 在该维度的表现)                           │
│  • item_b_summary (实体 B 在该维度的表现)                           │
│  • key_difference (关键差异)                                        │
│  • better_for (哪个更好: A/B/Both/Neither)                         │
│  • optional_score_a/b (0-10 分评分)                                │
└─────────────────────────────────────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────────┐
│                PHASE 4: 综合与判决 (Synthesis & Verdict)            │
│                                                                     │
│  ┌─────────────────────┐              ┌─────────────────────┐      │
│  │  ProsConsAgent      │              │ RecommendationAgent │      │
│  │                     │   并发执行    │                     │      │
│  │ • 提取优缺点         │   ═══════>   │ • 最终判决           │      │
│  │ • item_a_pros       │              │ • 使用场景建议       │      │
│  │ • item_a_cons       │              │ • 选择指导           │      │
│  │ • item_b_pros       │              │ • short_verdict     │      │
│  │ • item_b_cons       │              │ • long_verdict      │      │
│  └─────────────────────┘              └─────────────────────┘      │
└─────────────────────────────────────────────────────────────────────┘
                             │
                             ▼
...
1. **克隆仓库**
```bash
git clone git@github.com:Kenneth0416/Universal-Compare.git
cd universal-compare
```
...
```
用户输入 (Entity A, Entity B)
    ↓
ResearcherAgent × 2 (并发)
    ↓
Entity Profiles (结构化数据)
    ↓
ArchitectAgent
    ↓
Comparison Framework (4-6 维度)
    ↓
AnalystAgent × N (限制并发 3)
    ↓
Dimension Analysis (每个维度的详细分析)
    ↓
ProsConsAgent + RecommendationAgent (并发)
    ↓
Final Report (完整比较报告)
    ↓
UI 渲染 (React 组件)
...
- GitHub Issues: [提交 Issue](https://github.com/Kenneth0416/Universal-Compare/issues)
